...
//     alert(`BVM/B309/${componentName}/${index + 1}`);
//   };
...
// export default function B309LabLayout1() {
//   return (
//     <LabLayoutRoot>
//       <Box className="breadcrumb">
//         <Breadcrumb
//           routeSegments={[{ name: 'Labs', path: '/labs' }, { name: 'B309' }]}
//         />
//       </Box>

//       <SimpleCard title="B309">
...
const PC = ({ rotated, rotate270, index }) => {
  ...
  const fetchProductInfo = async (category) => {
    try {
      ...
      const productId = `${index + 1}_40`; 
      const response = await axios.get(`http://localhost:5000/api/product/${productId}/${category}/B309`, { headers: header });
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={handleClose} aria-labelledby="product-modal-title" aria-describedby="product-modal-description">
        <Box
          sx={{
            position: 'absolute',
            top: '50%',
...
export default function B309LabLayout1() {
  return (
    <LabLayoutRoot>
      <Box className="breadcrumb">
        <Breadcrumb routeSegments={[{ name: 'Labs', path: '/labs' }, { name: 'B309' }]} />
      </Box>
